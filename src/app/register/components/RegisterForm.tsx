...
export function RegisterForm() {
  ...
  const handleLogin = async () => {
    ...
    const password_confirm = confirmPasswordRef.current?.value;

    if (!username || !password || !name || !email || !password_confirm) {
      setError("Ingresa todos los campos para continuar");
      return;
    }
    ...
    if (password.length < 8) {
      setError("La contraseña debe ser por lo menos de 8 letras");
      return;
    }
...
function NameInput({
  ...
  return (
    <div className="flex flex-col w-full mb-4">
      <p className="mb-4 text-lg text-slate-200">NOMBRE COMPLETO</p>
      <div
        ref={ref}
        tabIndex={-1}
      ...
      >
        <User className="mr-4 size-10 min-h-10 min-w-10 text-slate-300" />
        <input
          ref={inputRef}
          type="text"
...
function UsernameInput({
  ...
  const { ref, focused } = useFocusable({
    focusKey: "REGISTER_USERNAME",
    onEnterPress: () => inputRef.current?.focus(),
  });

  return (
    <div className="flex flex-col w-full mb-4">
      <p className="mb-4 text-lg text-slate-200">USERNAME</p>
      <div
        ref={ref}
        tabIndex={-1}
        className={`flex p-3 border-[3px] rounded-xl bg-slate-700/50 transition-all duration-300 ring-4 border-slate-600 outline-none ${
...
function EmailInput({
  ...
  const { ref, focused } = useFocusable({
    focusKey: "REGISTER_EMAIL",
    onEnterPress: () => inputRef.current?.focus(),
  });

  return (
    <div className="flex flex-col w-full mb-4">
      <p className="mb-4 text-lg text-slate-200">CORREO ELECTRÓNICO</p>
      <div
        ref={ref}
        tabIndex={-1}
        className={`flex p-3 border-[3px] rounded-xl bg-slate-700/50 transition-all duration-300 ring-4 border-slate-600 outline-none ${
...
function ConfirmPasswordInput({
  ...
  const { ref, focused } = useFocusable({
    focusKey: "REGISTER_CONFIRM_PASSWORD",
    onEnterPress: () => inputRef.current?.focus(),
  });

  return (
    <div className="flex flex-col w-full mb-4">
      <p className="mb-4 text-lg text-slate-200">CONFIRMAR CONTRASEÑA</p>
      <div
        ref={ref}
        tabIndex={-1}
        className={`flex gap-4 p-3 border-[3px] rounded-xl bg-slate-700/50 transition-all duration-300 ring-4 border-slate-600 outline-none ${
